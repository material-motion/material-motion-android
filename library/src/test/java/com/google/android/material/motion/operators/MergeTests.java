...
public class MergeTests {
  @Test
  public void testValuesAreEmitted() {
    TrackingMotionObserver<Integer> tracker = new TrackingMotionObserver<>();
    ReactiveProperty<Integer> property = ReactiveProperty.of(0);
    ReactiveProperty<Integer> otherProperty = ReactiveProperty.of(1);
    property.getStream().compose(Merge.merge(otherProperty.getStream()))
      .subscribe(tracker);

    property.write(-10);
    otherProperty.write(5);
    assertThat(tracker.values).isEqualTo(Arrays.asList(0, 1, -10, 5));
  }
}
